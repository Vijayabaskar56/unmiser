...
```tsx
import { useEffect } from "react";
import { useLLM, models, DEFAULT_SYSTEM_PROMPT, LLMTool, ToolCall } from "react-native-executorch";
...
useEffect(() => {
  ...
  llm.configure({
    chatConfig: {
      systemPrompt: `Parse the user's message and return JSON. Don't reply to the user. ${instructions} /no_think`,
    },
  });
}, []);
